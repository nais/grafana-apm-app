...
> Complement to #85 (platform pipeline gaps). That issue asks for
...
| 14  | **Distinct sessions-affected count**                                                            | retry ladder range→1h→15m→5m→1m because `count(hash×session_id)` blows `max_query_series` (`exceptions.go` `sessionsFallbackWindows`; `sessions.go:110-116`)    | LogQL has **no** native distinct-count; `max_query_series:5000` / `volume_max_series:5000` are exact (`loki:158,162`). `approx_topk` approximates topk, not distinct-count                               | no existing-feature fix — **needs the recording rule** already requested in #85 (P1.2) | —      | (High, but blocked)            | B      |
