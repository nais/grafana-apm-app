...
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `tab`         | `overview` \| `issues` \| `alerts` \| `backend` \| `frontend` \| `database` \| `dependencies` \| `traces` \| `logs` \| `profiling` | Active tab on the service detail page. `overview` is the default and is stored by **omitting** the param. `backend` is the merged Endpoints (RED) + Runtime (USE) tab (docs/ia-review-2.md). **Legacy aliases** `tab=server` and `tab=runtime` (the former Endpoints and Runtime tabs) resolve to `tab=backend` **forever** — old bookmarks, links, and annotations keep working. `overview`, `alerts`, `backend`, `frontend`, and `database` are **always shown** (they render an empty/setup state when there is no data yet). The remaining values are **capability-gated** and silently fall back to `overview` when their datasource is absent: `issues`/`logs` need Loki, `traces` needs Tempo, `dependencies` needs service-graph metrics, `profiling` needs Pyroscope. |
...
| Param                | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `issueId`            | Fingerprint-based issue identity (#62), e.g. `v1:9f2ab31c04d7e655`. The primary drawer key, and it opens the drawer for **both** sources (#84): the hook resolves it against the unified issues list (`/issues`) for the issue's `source` + member hashes. Browser issues query Loki by hash; server issues have no Alloy hash and instead load occurrences from `/issues/occurrences` (re-scanned by recomputed fingerprint). A server `issueId` no longer redirects to the Logs tab — it opens the same drawer. |
| `exceptionHash`      | Legacy identity: a single Faro/Alloy `hash`. Still resolves (opens the drawer scoped to that one hash) — keep supporting it forever; old alert annotations and shared links use it. Browser-only.                                                                                                                                                                                                                                                                                                                 |
| `exceptionSessionId` | Selected session inside the drawer (optional, browser issues only). Never meaningful without `issueId`/`exceptionHash`.                                                                                                                                                                                                                                                                                                                                                                                           |
