...
import { Alert, Button, Field, FieldSet, IconButton, Input, SecretInput, Select, useStyles2 } from '@grafana/ui';
      ...
      <FieldSet label="Per-Environment Datasources" className={s.marginTop}>
        ...
        {state.envOverrides.map((ov, idx) => (
          <div key={idx} className={s.envRow}>
            <Field
              label="Environment"
              description={idx === 0 ? 'Must match the service_namespace label in your OTel data' : undefined}
            >
